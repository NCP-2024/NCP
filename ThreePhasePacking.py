...
from backend.utils.b2d.b2d import Box2DSimulator, get_concave
from backend.utils.EPD import EPD
...
class ThreePhasePacking(object):
    def __init__(self, data):
        ...
        self.extra_params['gamma'] = 1 #0.05
        self.extra_params['lambda'] = 0
    ...
    def __str__(self):
        ...
        for key in self.extra_params:
            if key not in ['top_iters', 'cpd_iters', 'alpha_min', 'rotation_angle', 'utopian', 'chebyshev']:
            ...
            for ang_size in range(self.extra_params['rotation_angle'], 0, -2):
                for ang_sign in [1, -1]:
                    ...
                    if s <= best_scaling:
                        ...
                        best_scaling = scaling_min
                        best_angle = ang
            # print(best_scaling, best_angle)
            return best_scaling, best_angle

        if cfg == 'Force-H-CPD' or cfg == "Sep-Force-H-CPD" or cfg == "Adjusted-Force-H-CPD":
            ...
            top_level_iterations = 1#00
            # top_level_iterations = 50
            ...
            try:
                top_level_diagram = self.extra_params['top_level_diagram']
            except:
                top_level_diagram = None
            ...
            for _ in range(top_level_iterations):
                ...
                if top_level_diagram is None or top_level_diagram == "CCPD":
                ...
                elif top_level_diagram == "CVD":
                    cells = computePowerDiagramByCGAL(positions=cluster_centers, hull=hull)
                elif top_level_diagram == "CPD-radii":
                    cells = computePowerDiagramByCGAL(positions=cluster_centers, weights=cluster_radii ** 2, hull=hull)
                elif top_level_diagram == "CPD-area":
                    capacity = cluster_weights / np.sum(cluster_weights) * cellArea(hull)
                    cells = computePowerDiagramByCGAL(positions=cluster_centers, weights=capacity, hull=hull)
            ...
            opt = None
            if cfg == "Force-H-CPD":
                opt = single_cell_optimization_force
            elif cfg == "Sep-Force-H-CPD":
                opt = single_cell_optimization_force_sep
            elif cfg == "Adjusted-Force-H-CPD":
                opt = single_cell_optimization_force_adjusted
            ...
            expected_mean_radii = 0.004
            if mean_radii < expected_mean_radii:
                radii = radii * expected_mean_radii / mean_radii
            _, _, new_pairs = computePowerDiagramBruteForce(positions=pos)
            for i, j in new_pairs:
                # if i j not belong to the same cluster, add them to the pairs
            ...
            mean_radii = np.mean(radii)
            expected_mean_radii = 1e-4
            if mean_radii < expected_mean_radii:
                radii = radii * expected_mean_radii / mean_radii * 2

            _, _, pairs = computePowerDiagramBruteForce(positions=pos, clipHull=False)
            circle_list = [[pos[i, 0], pos[i, 1], radii[i]] for i in range(self.N)]
            preservation = set(pairs)
